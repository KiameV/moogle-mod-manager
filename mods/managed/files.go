...
func AddModFiles(game config.Game, tm *mods.TrackedMod, files []*mods.DownloadFiles) (err error) {
	...
	for _, df := range files {
		...
		if err = MoveFiles(df.Files, modDir, configs.GetGameDir(game), configs.GetBackupFullPath(game), &backedUp, &moved, false); err != nil {
			break
		}
		if err == nil {
			if err = MoveDirs(game, df.Dirs, modDir, configs.GetGameDir(game), configs.GetBackupFullPath(game), &backedUp, &moved, false); err != nil {
				break
			}
		}
	}
